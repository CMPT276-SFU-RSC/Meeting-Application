...
public interface PollRepository extends JpaRepository<Poll, Integer> {
    ...
    @Query(
        value = "SELECT DISTINCT user_responses.uid " + 
                "FROM user_responses, polls " + 
                "WHERE user_responses.pid = ?1 AND polls.pid = user_responses.pid",
        nativeQuery = true
    )
    ...
    @Query(
        value = "SELECT DISTINCT invited.uid " + 
                "FROM invited, polls " + 
                "WHERE invited.pid = ?1 AND polls.pid = invited.pid",
        nativeQuery = true
    )
